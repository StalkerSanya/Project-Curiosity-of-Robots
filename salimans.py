...
## Class to get dataset of observations and labels(from NET_1) 
## in tensor form for collection of training set (use for more faster training)
class BehaviourDataset(Dataset):
    def __init__(self, observation_list, labels_list):
        
        self.observations = observation_list
        self.labels = labels_list

    def __len__(self):
        if len(self.labels) == len(self.observations):
            return len(self.observations)
        else:
          print("ERROR IN SIZES OF labels and observations")

    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()

        observe = self.observations[idx]
        label = self.labels[idx]

        #convert observation from numpy to tensor
        tensor_behave = observe
        tensor_label = label
        return tensor_behave, tensor_label


##First Network (Target network)
class NET_1(nn.Module):
    def __init__(self, number_of_inputs, number_of_outputs = 5, number_of_hiddens = 50):
        super(NET_1, self).__init__()
      
        # Inputs to hiddens layers linear transformation
        self.hidden_1 = nn.Linear(number_of_inputs, 50) #25088
        nn.init.xavier_normal_(self.hidden_1.weight, gain = torch.nn.init.calculate_gain('sigmoid'))
        self.hidden_2 = nn.Linear(number_of_hiddens, number_of_hiddens)
        nn.init.xavier_normal_(self.hidden_2.weight, gain = torch.nn.init.calculate_gain('sigmoid'))
        # Output layer
        self.output = nn.Linear(number_of_hiddens, number_of_outputs)
        nn.init.xavier_normal_(self.output.weight, gain = torch.nn.init.calculate_gain('sigmoid'))
...
##Second Network (Predictive Network)
